# 12. Transformer Currents
A transformer has a primary coil with 1000 turns and a secondary coil with 200 turns. If the primary voltage is $120\text{ V}$ (AC), what is the secondary voltage? If the current in the secondary is $3\text{ A}$, what is the current in the primary?
## Useful definitions and formulas

For an ideal transformer:

transformer ratio formula
$$\frac{V_s}{V_p}=\frac{N_s}{N_p}$$
It shows the relationship between voltage and the number of turns in the coils.

Also, power is conserved:
conservation of power formula
$$V_p I_p=V_s I_s$$
This formula means that the input power is equal to the output power in an ideal transformer.

where:

$V_p$ — primary voltage  
$V_s$ — secondary voltage  
$N_p$ — primary turns  
$N_s$ — secondary turns  
$I_p$ — primary current  
$I_s$ — secondary current  

---

## Step-by-step solution

Given:

$$N_p=1000$$

$$N_s=200$$

$$V_p=120\,V$$

$$I_s=3\,A$$

### Secondary voltage

$$\frac{V_s}{120}=\frac{200}{1000}$$

$$V_s=120\cdot0.2$$

$$V_s=24\,V$$

### Primary current

Using power conservation:

$$V_pI_p=V_sI_s$$

$$120I_p=24\cdot3$$

$$120I_p=72$$

$$I_p=\frac{72}{120}$$

$$I_p=0.6\,A$$

---

## Final answer

Secondary voltage:

$$V_s=24\,V$$

Primary current:

$$I_p=0.6\,A$$
